...
object Throttle {
  ...
  object TokenBucket {
    ...
    def local[F[_]](capacity: Int, refillEvery: FiniteDuration)(
      ...
      //FIXME calculate retry-after time - tests
      bucket.map { counter =>
        new TokenBucket[F] {
  ...
  /**
    ...
    * @return a task containing the transformed service.
    */
  def apply[F[_], G[_]](bucket: TokenBucket[F], throttleResponse: Option[FiniteDuration] => Response[G] = defaultResponse[G] _)(http: Http[F, G])(
